...
export const getImageForLocation = (locationName) => {
    ...
    // Exact match check
    if (LOCATION_IMAGES[normalizedName]) {
        return LOCATION_IMAGES[normalizedName];
    }

    // Partial match check: Find the specific/longest match
    ...
    for (const key of keys) {
        if (normalizedName.includes(key) || key.includes(normalizedName)) {
            // If we find a match, check if it's more specific (longer) than previous matches
            if (key.length > maxLen) {
                bestMatch = LOCATION_IMAGES[key];
                maxLen = key.length;
            }
        }
    }
